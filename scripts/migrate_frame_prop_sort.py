...
async def migrate_space(conn, space_id: str, apply: bool) -> dict:
    ...
    if exists and apply:
        ...
        # `form_type_uuid` likewise: a table from the Assertion-only revision
        # does not have it, and the populate below writes it. Without this the
        # INSERT fails on a column that does not exist.
        await conn.execute(
            f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS form_type_uuid UUID")
    ...
    all_types = await frame_prop_sort_coverage(conn, space_id, limit=500, only_gaps=False)
    for cov in all_types:
        await record_prop_sort_coverage(
            conn, space_id, cov["frame_type_uuid"], cov["in_table"], cov["of_type"])
