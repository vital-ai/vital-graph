-- Corrected optimized frame query using materialized view
-- This aligns with the original working query logic
...
-- OPTIMIZED: Use materialized view but map to original query logic
-- In the original: source.object_uuid = frame_uuid, dest.object_uuid = slot_uuid
edge_structure AS MATERIALIZED (
    SELECT 
        mv.edge_uuid,
        mv.source_node_uuid as frame_uuid,  -- This matches original logic
        mv.dest_node_uuid as slot_uuid      -- This matches original logic
    FROM vitalgraph1__wordnet_frames__edge_relationships_mv mv
),
-- Get slot types and entities with early happy entity filtering (same as original)
...
-- Combine the results (same logic as original)
final_results AS (
    SELECT DISTINCT
        es.frame_uuid,
        sd.entity_uuid,
