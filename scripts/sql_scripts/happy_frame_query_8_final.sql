-- Final optimized frame query using materialized view for edge relationships
-- Properly aligned with the node/edge data model: frames, slots, entities are nodes; edges connect them
...
-- OPTIMIZED: Use materialized view to find frame-slot relationships via edges
frame_slot_relationships AS MATERIALIZED (
    SELECT DISTINCT
        mv.source_node_uuid as frame_uuid,
        mv.dest_node_uuid as slot_uuid,
        mv.edge_uuid
    FROM vitalgraph1__wordnet_frames__edge_structure_mv mv
),
...
-- Combine frame-slot relationships with slot data
final_results AS (
    SELECT DISTINCT
        fsr.frame_uuid,
        sd.entity_uuid,
        ...
        END as slot_role
    FROM constants
    CROSS JOIN frame_slot_relationships fsr
    JOIN slot_data sd ON sd.slot_uuid = fsr.slot_uuid
),
