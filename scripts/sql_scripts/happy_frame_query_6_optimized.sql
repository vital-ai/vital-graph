-- Optimized frame query using materialized view for edge structure
-- This version leverages pre-computed edge relationships for better performance
...
-- OPTIMIZED: Use materialized view for edge structure (much faster!)
edge_structure_fast AS MATERIALIZED (
    SELECT 
        edge_uuid,
...
-- Get slot types and entities with early happy entity filtering (same optimization as before)
...
-- Combine results using the fast edge structure
final_results AS (
    SELECT DISTINCT
        es.frame_uuid,
        sd.entity_uuid,
        ...
        END as slot_role
    FROM constants
    CROSS JOIN edge_structure_fast es  -- Using the fast materialized view!
