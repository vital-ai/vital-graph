...
async def survey_space(conn, space_id: str) -> dict | None:
    ...
    missing = [r for r in rows if not r["has_any"]]
    return {"space": space_id, "pred": pred, "total": total,
            "missing": missing, "misdirected": misdirected}
    ...
    try:
        ...
        for sid in targets:
            ...
            logger.info("%s: wrote %d self-link(s) in %.1fs%s", sid, wrote,
                        time.time() - t0,
                        "" if not s["misdirected"] else
                        f"; {len(s['misdirected'])} misdirected left alone")
