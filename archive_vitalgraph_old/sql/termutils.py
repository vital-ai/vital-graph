...
def normalize_graph(graph):
    ...
    """
    _logger.info(f"🚀 normalize_graph: FUNCTION STARTED with graph={graph}")
...
def term_to_letter(term):
    ...
    * :class:`~rdflib.graph.QuotedGraph`
    ...
    'L'
    """
    _logger.info(f"🚀 term_to_letter: FUNCTION STARTED with term={term}")
...
def construct_graph(key):
    ...
    """
    _logger.info(f"🚀 construct_graph: FUNCTION STARTED with key={key}")
...
def triple_pattern_to_term_combinations(triple):
    ...
    _logger.info(f"🚀 triple_pattern_to_term_combinations: FUNCTION STARTED with triple={triple}")
    s, p, o = triple
    combinations = []
...
def type_to_term_combination(member, klass, context):
    ...
    _logger.info(f"🚀 type_to_term_combination: FUNCTION STARTED with member={member}, klass={klass}, context={context}")
...
def statement_to_term_combination(subject, predicate, obj, context):
    ...
    _logger.info(f"🚀 statement_to_term_combination: FUNCTION STARTED with subject={subject}, predicate={predicate}, obj={obj}, context={context}")
...
def escape_quotes(qstr):
    ...
    """
    _logger.info(f"🚀 escape_quotes: FUNCTION STARTED with qstr length={len(qstr) if qstr else 0}")
    if qstr is None:
        return ""
    tmp = qstr.replace("\\", "\\\\")
    tmp = tmp.replace("'", "\\'")
...
def extract_triple(tupleRt, store, hardCodedContext=None):
    ...
    """
    _logger.info(f"🚀 extract_triple: FUNCTION STARTED with tupleRt length={len(tupleRt) if tupleRt else 0}")
    try:
...
def create_term(termString, termType, store, objLanguage=None, objDatatype=None):
    ...
    """
    _logger.info(f"🚀 create_term: FUNCTION STARTED with termString={termString}, termType={termType}, objLanguage={objLanguage}, objDatatype={objDatatype}")
    if termType == "L":
        ...
        if cache is not None:
            ...
            if objLanguage and not objDatatype:
                ...
                # RDF literals can only have lang OR datatype, not both
                if objDatatype:
                    rt = Literal(termString, datatype=objDatatype)
                    store.literalCache[((termString, objDatatype))] = rt
                elif objLanguage:
