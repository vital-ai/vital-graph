...
class TableConfig:
    ...
    @classmethod
    def from_space_impl(cls, space_impl: PostgreSQLSpaceImpl, space_id: str) -> 'TableConfig':
...
class PostgreSQLSparqlImpl:
    ...
    def __init__(self, space_impl: PostgreSQLSpaceImpl):
        ...
        self.join_counter = 0
        self.term_uuid_cache = PostgreSQLCacheTerm(cache_size=10000)  # Cache for term UUID lookups
        ...
        for triple_idx, triple in enumerate(triples):
            ...
            if isinstance(predicate, Path):
                ...
                if term_key in term_uuid_mappings:
                    ...
                    self.logger.debug(f"🔍 BIND+OPTIONAL DEBUG: Searched for predicate key: {term_key}")
                    self.logger.debug(f"🔍 BIND+OPTIONAL DEBUG: Available term mappings ({len(term_uuid_mappings)}): {list(term_uuid_mappings.keys())}")
                    self.logger.debug(f"🔍 BIND+OPTIONAL DEBUG: All bound terms collected: {bound_terms}")
                    all_where_conditions.append("1=0")  # Condition that never matches
