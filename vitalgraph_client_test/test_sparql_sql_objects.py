#!/usr/bin/env python3
"""
SPARQL-SQL Backend Objects Test

Standalone runner for the Objects CRUD test case against the sparql_sql backend.
Creates a dedicated space, runs objects tests, then cleans up.
"""
...
# Configuration
# ===========================================================================
TEST_SPACE_ID = "sp_sql_obj"
TEST_SPACE_NAME = "SPARQL-SQL Objects Test Space"
TEST_GRAPH_ID = "urn:sql_objects"
...
async def main():
    print("\n" + "=" * 80)
    print("  SPARQL-SQL Backend — Objects CRUD Test")
    print("=" * 80)
    ...
    try:
        ...
        space = Space(space=TEST_SPACE_ID, space_name=TEST_SPACE_NAME,
                      space_description="Automated objects CRUD test space")
        ...
        # Create graph
        gr = await client.graphs.create_graph(TEST_SPACE_ID, TEST_GRAPH_ID)
        logger.info(f"  ✅ Graph '{TEST_GRAPH_ID}' created")

        # Run objects tests
        tester = ObjectsCrudTester(client)
        results = tester.run_tests(TEST_SPACE_ID, TEST_GRAPH_ID)
