...
# ---------------------------------------------------------------------------
# Connection parameters — MOVED to `devtools.target`
# ---------------------------------------------------------------------------
#
# This package is experimental and excluded from the wheel; the stack resolver is
# depended on by 25 scripts and the conformance suite, so it must not live here.
# Re-exported so existing imports keep working. Import from `devtools.target` in
# new code.
from devtools.target import (  # noqa: F401
    _SIDECAR_DEFAULT,
    _STACK_DEFAULTS,
    add_pg_arguments,
    describe_target,
