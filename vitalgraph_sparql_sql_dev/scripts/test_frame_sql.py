...
WITH happy_entities AS (
    ...
    WHERE obj.term_text ILIKE '%happy%'
        AND obj.term_type = 'L'
)
-- Source entity is happy
SELECT 
...
"""
    ...
    async def analyze_table_structure(self, conn, space_id: str = "wordnet_exp"):
        ...
        cursor.execute(f"""
            ...
            AND attname IN ('term_uuid', 'subject_uuid', 'predicate_uuid', 'object_uuid', 'context_uuid', 'term_text')
...
def main():
    ...
    try:
        ...
        # Phase 1: Fetch constants
        t0 = time.time()
        constants = benchmark.fetch_constants()
        constants_ms = (time.time() - t0) * 1000
        timings.append(("Fetch constants", constants_ms))

        if not constants:
            logger.error("Failed to fetch constants")
            return 1
...
WITH happy_entities AS (
    ...
    FROM {term_table} obj
        JOIN {quad_table} q1
            ON q1.object_uuid = obj.term_uuid
            AND q1.predicate_uuid = '{benchmark.constants['has_description_uuid']}'
            AND q1.context_uuid = '{benchmark.constants['graph_uuid']}'
        JOIN {quad_table} q2
            ON q2.subject_uuid = q1.subject_uuid
            AND q2.predicate_uuid = '{benchmark.constants['rdf_type_uuid']}'
            AND q2.object_uuid = '{benchmark.constants['kg_entity_uuid']}'
            AND q2.context_uuid = '{benchmark.constants['graph_uuid']}'
    WHERE obj.term_text ILIKE '%happy%'
        AND obj.term_type = 'L'
)
SELECT
...
{quad_table} ev1
    JOIN {quad_table} st1
        ON st1.subject_uuid = ev1.subject_uuid
        AND st1.predicate_uuid = '{benchmark.constants['has_slot_type_uuid']}'
        AND st1.object_uuid = '{benchmark.constants['has_source_entity_uuid']}'
        AND st1.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} ed1
        ON ed1.object_uuid = ev1.subject_uuid
        AND ed1.predicate_uuid = '{benchmark.constants['has_edge_destination_uuid']}'
        AND ed1.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} es1
        ON es1.subject_uuid = ed1.subject_uuid
        AND es1.predicate_uuid = '{benchmark.constants['has_edge_source_uuid']}'
        AND es1.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} ev2
        ON ev2.predicate_uuid = '{benchmark.constants['has_entity_slot_value_uuid']}'
        AND ev2.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} st2
        ON st2.subject_uuid = ev2.subject_uuid
        AND st2.predicate_uuid = '{benchmark.constants['has_slot_type_uuid']}'
        AND st2.object_uuid = '{benchmark.constants['has_destination_entity_uuid']}'
        AND st2.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} ed2
        ON ed2.object_uuid = ev2.subject_uuid
        AND ed2.predicate_uuid = '{benchmark.constants['has_edge_destination_uuid']}'
        AND ed2.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} es2
        ON es2.subject_uuid = ed2.subject_uuid
        AND es2.predicate_uuid = '{benchmark.constants['has_edge_source_uuid']}'
        AND es2.object_uuid = es1.object_uuid
        AND es2.context_uuid = '{benchmark.constants['graph_uuid']}'
    CROSS JOIN LATERAL (
        SELECT ev1.subject_uuid as slot_uuid,
               ev1.object_uuid as entity_uuid,
               es1.object_uuid as frame_uuid
    ) fc1
    CROSS JOIN LATERAL (
        SELECT ev2.subject_uuid as slot_uuid,
               ev2.object_uuid as entity_uuid
    ) fc2
WHERE ev1.predicate_uuid = '{benchmark.constants['has_entity_slot_value_uuid']}'
    AND ev1.context_uuid = '{benchmark.constants['graph_uuid']}'
    AND EXISTS (SELECT 1 FROM happy_entities WHERE entity_uuid = ev1.object_uuid)
...
{quad_table} ev2
    ...
    JOIN {quad_table} es2
        ...
        AND es2.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} ev1
        ON ev1.predicate_uuid = '{benchmark.constants['has_entity_slot_value_uuid']}'
        AND ev1.context_uuid = '{benchmark.constants['graph_uuid']}'
    JOIN {quad_table} st1
        ON st1.subject_uuid = ev1.subject_uuid
        AND st1.predicate_uuid = '{benchmark.constants['has_slot_type_uuid']}'
    ...
    JOIN {quad_table} es1
        ON es1.subject_uuid = ed1.subject_uuid
        AND es1.predicate_uuid = '{benchmark.constants['has_edge_source_uuid']}'
        AND es1.object_uuid = es2.object_uuid
        AND es1.context_uuid = '{benchmark.constants['graph_uuid']}'
    CROSS JOIN LATERAL (
        SELECT ev1.subject_uuid as slot_uuid,
               ev1.object_uuid as entity_uuid,
...
WHERE ev2.predicate_uuid = '{benchmark.constants['has_entity_slot_value_uuid']}'
    AND ev2.context_uuid = '{benchmark.constants['graph_uuid']}'
    AND EXISTS (SELECT 1 FROM happy_entities WHERE entity_uuid = ev2.object_uuid)
...
"""
        ...
        with benchmark.connection.cursor() as cursor:
            t0 = time.time()
            cursor.execute(hardcoded_sql)
            results_hc = cursor.fetchall()
            hc_ms = (time.time() - t0) * 1000
            timings.append(("Full frame query (hardcoded UUIDs)", hc_ms))
            logger.info(f"  {len(results_hc)} rows in {hc_ms:.1f}ms")

        # ---- Timing Summary ----
        print("\n" + "=" * 80)
        print("FRAME SQL BENCHMARK — Timing Summary")
        print("=" * 80)
        print(f"  {'Phase':<45} {'Time':>10}")
        print(f"  {'-'*45} {'-'*10}")
        total_ms = 0.0
        for label, ms in timings:
            print(f"  {label:<45} {ms:9.1f}ms")
            total_ms += ms
        print(f"  {'-'*45} {'-'*10}")
        print(f"  {'TOTAL':<45} {total_ms:9.1f}ms")

        print(f"\n  Hardcoded variant rows: {len(results_hc)}")
        if results_hc[:3]:
            print(f"\n  Sample results (first 3):")
            for i, row in enumerate(results_hc[:3], 1):
                print(f"    {i}: entity={row[0]}, frame={row[1]}")
        print("=" * 80)
