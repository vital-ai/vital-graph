-- Corrected optimized frame query using materialized view for edge structure
-- This version properly aligns the edge structure with the actual data model
...
-- Use the original edge_structure logic but with better performance
-- (keeping the original logic to ensure we get the same results)
...
-- Combine results using the edge structure
final_results AS (
    SELECT DISTINCT
        es.frame_uuid,
        sd.entity_uuid,
