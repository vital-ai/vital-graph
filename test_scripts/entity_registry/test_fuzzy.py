...
Tests the EntityFuzzyIndex (datasketch MinHash LSH + RapidFuzz)
without the REST layer — uses asyncpg directly.

Usage:
    /opt/homebrew/anaconda3/envs/vital-graph/bin/python test_scripts/entity_registry/test_fuzzy.py
"""
...
class FuzzyTestRunner:
    ...
    async def setup(self):
        """Create registry with fuzzy index and seed test entities."""
        import asyncpg
        ...
        fuzzy_index = EntityFuzzyIndex(num_perm=128, threshold=0.3)
        self.registry = EntityRegistryImpl(pool, fuzzy_index=fuzzy_index)
        await self.registry.ensure_tables()
        logger.info("Registry with fuzzy index initialized")
    ...
    async def create_test_entity(self, name, type_key='business', country=None,
        ...
        entity = await self.registry.create_entity(
            ...
            aliases=alias_dicts, created_by='fuzzy_test',
        )
    ...
    async def cleanup(self):
        ...
        for eid in self.created_entity_ids:
            try:
                await self.registry.delete_entity(eid, deleted_by='fuzzy_test')
            except Exception:
                pass
    ...
    async def test_exact_name_match(self):
        ...
        results = await self.registry.find_similar("Acme Corporation")
    ...
    async def test_different_entities(self):
        ...
        results = await self.registry.find_similar("Zebra Logistics International", min_score=80.0)
        for r in results:
    ...
    async def test_location_influence(self):
        ...
        results_us = await self.registry.find_similar("Smith & Associates", country='US')
        results_uk = await self.registry.find_similar("Smith & Associates", country='UK')
    ...
    async def test_index_add_remove(self):
        logger.info("\n--- Index Add/Remove ---")
        fuzzy = self.registry.fuzzy_index
        initial_count = fuzzy.entity_count

        e1 = await self.create_test_entity("Temporary Test Corp", country='US')
        self.check("Entity added to index", fuzzy.entity_count == initial_count + 1,
                    f"count={fuzzy.entity_count}")

        # Find it
        results = await self.registry.find_similar("Temporary Test Corp")
        found_ids = [r['entity_id'] for r in results]
        ...
        self.check("Entity removed from index", fuzzy.entity_count == initial_count,
                    f"count={fuzzy.entity_count}")
    ...
    async def test_min_score_filter(self):
        ...
        results_low = await self.registry.find_similar("Global Solutions", min_score=30.0)
        results_high = await self.registry.find_similar("Global Solutions", min_score=90.0)
    ...
    async def test_empty_query(self):
        logger.info("\n--- Empty/Short Query ---")
        results = await self.registry.find_similar("")
    ...
    async def test_find_duplicates_for_entity(self):
        ...
        results = await self.registry.find_duplicates_for_entity(entity, limit=5)
        found_ids = [r['entity_id'] for r in results]
        self.check("Finds near-duplicate", e2['entity_id'] in found_ids,
                    f"results={len(results)}")
        self.check("Excludes self", e1['entity_id'] not in found_ids)

    async def test_fuzzy_index_stats(self):
        logger.info("\n--- Fuzzy Index Stats ---")
        fuzzy = self.registry.fuzzy_index
        self.check("Entity count > 0", fuzzy.entity_count > 0,
                    f"count={fuzzy.entity_count}")
        self.check("Index is initialized", fuzzy._initialized is True)

    async def run_all(self):
        logger.info("=" * 60)
        logger.info("Entity Fuzzy Index Tests")
        logger.info("=" * 60)
        ...
        try:
            ...
            await self.test_fuzzy_index_stats()
        finally:
            await self.cleanup()
