#!/usr/bin/env python3
"""
Standalone unit tests for EntityFuzzyIndex (MinHash LSH + RapidFuzz).

No database, no server, no network — tests the fuzzy logic purely in-memory.

Usage:
    /opt/homebrew/anaconda3/envs/vital-graph/bin/python test_scripts/entity_registry/test_fuzzy_standalone.py
"""
...
from vitalgraph.entity_registry.entity_fuzzy import EntityFuzzyIndex, build_shingles

logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(name)s: %(message)s')
logger = logging.getLogger(__name__)


class StandaloneFuzzyTests:
    """Pure in-memory tests for EntityFuzzyIndex."""
    ...
    def _fresh_index(self) -> EntityFuzzyIndex:
        """Create a fresh in-memory fuzzy index."""
        return EntityFuzzyIndex(num_perm=128, threshold=0.3)

    def _seed_index(self, index: EntityFuzzyIndex):
    ...
    def run_all(self) -> bool:
        logger.info("=" * 60)
        logger.info("EntityFuzzyIndex Standalone Unit Tests")
