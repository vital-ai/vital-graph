#!/usr/bin/env python3
"""
Standalone tests for EntityFuzzyIndex phonetic + typo extensions.
...
Uses the 100 test entities from test_fuzzy_entities.py.

Usage:
    /opt/homebrew/anaconda3/envs/vital-graph/bin/python test_scripts/entity_registry/test_fuzzy_extensions.py
"""
...
from test_scripts.entity_registry.test_fuzzy_entities import TEST_ENTITIES

logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s %(name)s: %(message)s')
logger = logging.getLogger(__name__)


class FuzzyExtensionTests:
    ...
    def _build_index(self) -> EntityFuzzyIndex:
        """Build index from all 100 test entities."""
        idx = EntityFuzzyIndex(num_perm=128, threshold=0.3, phonetic_bonus=10.0)
    ...
    def test_phonetic_codes_basic(self):
        ...
        codes = EntityFuzzyIndex._phonetic_codes("Schneider")
        self.check("Schneider has codes", len(codes) > 0, f"codes={codes}")

        codes2 = EntityFuzzyIndex._phonetic_codes("Snyder")
    ...
    def test_phonetic_codes_multi_word(self):
        ...
        codes = EntityFuzzyIndex._phonetic_codes("Johnson Partners")
    ...
    def test_phonetic_codes_short_word(self):
        ...
        codes = EntityFuzzyIndex._phonetic_codes("A B")
        self.check("Single-char words produce no codes", len(codes) == 0)

        codes2 = EntityFuzzyIndex._phonetic_codes("AB")
    ...
    def test_edit_distance_1_basic(self):
        ...
        variants = EntityFuzzyIndex._edit_distance_1("cat")
    ...
    def test_remove_cleans_phonetic_lsh(self):
        ...
        idx = EntityFuzzyIndex(num_perm=128, threshold=0.3)
        idx.add_entity('x1', {'primary_name': 'Uniquephonetic'})
        codes = EntityFuzzyIndex._phonetic_codes("Uniquephonetic")
        mh = idx._build_minhash(set(codes))
    ...
    def test_phonetic_bonus_value(self):
        ...
        idx_with = EntityFuzzyIndex(num_perm=128, threshold=0.3, phonetic_bonus=10.0)
        idx_without = EntityFuzzyIndex(num_perm=128, threshold=0.3, phonetic_bonus=0.0)
        for eid, data in TEST_ENTITIES:
    ...
    def run_all(self) -> bool:
        logger.info("=" * 60)
        logger.info("EntityFuzzyIndex Extension Tests (Phonetic + Typo)")
