...
class GeoCriteria:
    ...
    Generates SPARQL:
      BIND(vg:geoDistance(?anchor, lat, lon) AS ?vg_distance)
    and/or:
      FILTER(vg:withinRadius(?anchor, lat, lon, radius_m))

    Where ?anchor is determined by geo_target:
      - "entity" (default): binds to ?entity
      - "slot": binds to the geo slot variable from frame_criteria
    """
    latitude: float = 0.0
    longitude: float = 0.0
    ...
    geo_target: Optional[str] = None        # "slot" | "entity" | None (default=entity)
...
FILTER(CONTAINS(LCASE(?search_name), LCASE("{criteria.search_string}")))""")
    ...
    def _build_vector_geo_clauses(
        ...
        if geo_criteria:
            ...
            # Determine geo anchor variable based on geo_target
            geo_anchor = anchor_var  # default from caller (usually "entity")
            if gc.geo_target == "slot":
                # Use the first slot variable if available (e.g. "slot_0_0")
                # The slot variable is generated by frame criteria as slot_{frame_idx}_{slot_idx}
                geo_anchor = "slot_0_0"  # convention: first frame, first slot
            elif gc.geo_target == "entity" or gc.geo_target is None:
                geo_anchor = anchor_var
            
            # Distance calculation
            where_parts.append(
                f'BIND(vg:geoDistance(?{geo_anchor}, "{gc.latitude}"^^xsd:double, '
                f'"{gc.longitude}"^^xsd:double) AS ?{dist_var})')
            
            # Radius filter
            if gc.radius_m is not None:
                where_parts.append(
                    f'FILTER(vg:withinRadius(?{geo_anchor}, "{gc.latitude}"^^xsd:double, '
    ...
    def _build_document_where_clause(self, criteria: DocumentQueryCriteria) -> str:
        ...
        # Two cases: URI is the original doc (two-hop) or already a parent copy (one-hop)
        if criteria.parent_document_uri:
            filter_clauses.append(
                "{"
                "  ?_seg_e1 vital-core:hasEdgeSource <" + criteria.parent_document_uri + "> ."
                "  ?_seg_e1 vital-core:hasEdgeDestination ?_seg_parent ."
                "  ?_seg_parent haley:hasKGDocumentSegmentTypeURI <urn:segtype:segmentation_parent> ."
                "  ?_seg_e2 vital-core:hasEdgeSource ?_seg_parent ."
                "  ?_seg_e2 vital-core:hasEdgeDestination ?entity ."
                "} UNION {"
                "  ?_seg_edge vital-core:hasEdgeSource <" + criteria.parent_document_uri + "> ."
                "  ?_seg_edge vital-core:hasEdgeDestination ?entity ."
                "  ?entity haley:hasKGDocumentSegmentTypeURI ?_seg_st ."
                "  FILTER(?_seg_st != <urn:segtype:segmentation_parent>)"
                "}"
            )
        ...
        if criteria.search_text:
            ...
            if criteria.fts_index_name:
                ...
                filter_clauses.append(
                    f'BIND(vg:textSearch(?entity, "{escaped_text}", '
                    f'"{criteria.fts_index_name}") AS ?_fts_score)')
                filter_clauses.append("FILTER(?_fts_score > 0)")
            else:
                # Fallback: brute-force CONTAINS scan on headline/content
                filter_clauses.append(
                ...
                filter_clauses.append(
                    f'FILTER(CONTAINS(LCASE(STR(COALESCE(?_hl, ""))), LCASE("{escaped_text}"))'
                    f' || CONTAINS(LCASE(STR(COALESCE(?_ct, ""))), LCASE("{escaped_text}")))'
                )
        ...
        # Segmentation-aware OPTIONAL projections
    ...
    def build_document_query_sparql(self, criteria: DocumentQueryCriteria,
        ...
        # FTS score projection + ordering
        if criteria.search_text and criteria.fts_index_name:
            select_extra += " ?_fts_score"
            if not criteria.sort_criteria and not criteria.vector_criteria:
                order_by = "ORDER BY DESC(?_fts_score)"

        # Add enrichment projections to SELECT
        if criteria.include_segment_text:
            select_extra += " ?_seg_content ?_seg_headline"
        if criteria.include_parent_context:
            select_extra += " ?_parent_doc ?_parent_name"
        if criteria.include_original_uri:
            select_extra += " ?_original_doc"
    ...
    def _build_document_grouped_query(self, criteria: DocumentQueryCriteria,
        ...
        Uses SAMPLE(?entity) and MAX(?vg_score) aggregation per parent document.
        """
        # Vector/geo criteria for score variable
        vg_extra_where, _vg_select, _vg_order, vg_limit = self._build_vector_geo_clauses(
            criteria.vector_criteria, criteria.geo_criteria,
            multi_vector_criteria=criteria.multi_vector_criteria,
            anchor_var="entity")
        ...
        if vg_extra_where:
            full_where += " " + vg_extra_where

        # Build SELECT projections with aggregation
        select_parts = [
            "?_parent_doc",
            "(SAMPLE(?entity) AS ?_best_seg)",
            "(MAX(?vg_score) AS ?_max_score)",
            "(SAMPLE(?_parent_name) AS ?_parent_name_agg)",
        ]
        if criteria.include_original_uri:
            select_parts.append("(SAMPLE(?_original_doc) AS ?_original_doc_agg)")
        if criteria.include_segment_text:
            select_parts.append("(SAMPLE(?_seg_content) AS ?_seg_content_agg)")
            select_parts.append("(SAMPLE(?_seg_headline) AS ?_seg_headline_agg)")
    ...
    def build_document_count_query_sparql(self, criteria: DocumentQueryCriteria,
        ...
        if criteria.group_by_document:
            count_expr = "COUNT(DISTINCT ?_parent_doc)"
        else:
            count_expr = "COUNT(DISTINCT ?entity)"
