...
class VitalGraphImpl:
    def __init__(self, app: FastAPI, config=None):
        ...
        if self.config is None:
            try:
                ...
                # Log error but continue with None config (will use defaults)
        ...
        # Create VitalGraphAuth instance
        self.auth = VitalGraphAuth()
        
        # Create VitalGraphAPI instance with auth handler and database implementation
        self.api = VitalGraphAPI(self.auth, self.db_impl)
        ...
        # Setup startup event to connect to database
    ...
    def _setup_middleware(self):
        ...
        self.app.add_middleware(SessionMiddleware, secret_key=os.getenv("SECRET_KEY", "CHANGE_THIS"))
    
    def _setup_static_files(self):
        """Mount React frontend build files only in production"""
        if self.app_mode == "production":
            # Get the project root directory (go up from vitalgraph/impl to project root)
            project_root = Path(__file__).parent.parent.parent
            
            self.app.mount(
                "/assets",
                StaticFiles(directory=str(project_root / "vitalgraph" / "api" / "frontend" / "dist" / "assets")),
                name="static_assets"
            )
            ...
            self.app.mount(
                "/images",
                StaticFiles(directory=str(project_root / "vitalgraph" / "api" / "frontend" / "dist" / "images")),
                name="static_images"
            )
    ...
    def _setup_routes(self):
        ...
        # Create wrapper functions for routes that need authentication
        ...
        async def add_space_wrapper(space_data: Space, current_user: Dict = Depends(self.get_current_user)) -> Space:
            return await self.api.add_space(space_data.dict(exclude_unset=True), current_user)
        ...
        async def update_space_wrapper(space_id: int, space_data: Space, current_user: Dict = Depends(self.get_current_user)) -> Space:
            return await self.api.update_space(space_id, space_data.dict(exclude_unset=True), current_user)
        ...
        async def add_user_wrapper(user_data: User, current_user: Dict = Depends(self.get_current_user)) -> User:
            return await self.api.add_user(user_data.dict(exclude_unset=True), current_user)
        ...
        async def update_user_wrapper(user_id: int, user_data: User, current_user: Dict = Depends(self.get_current_user)) -> User:
            return await self.api.update_user(user_id, user_data.dict(exclude_unset=True), current_user)
        ...
        async def sparql_endpoint_wrapper(query: str = Body(...), current_user: Dict = Depends(self.get_current_user)):
        ...
        self.app.post(
            ...
            tags=["Authentication"],
            summary="User Login",
            description="Authenticate user and obtain access token for API access"
        )(self.login)
        self.app.post(
            "/api/logout",
            tags=["Authentication"],
        ...
        self.app.get(
            ...
            description="Retrieve a list of all graph spaces accessible to the current user",
        ...
        self.app.post(
            ...
            description="Create a new graph space with the provided configuration",
        ...
        self.app.get(
            ...
            description="Retrieve detailed information about a specific space by its ID",
            response_model=Space
        )(get_space_wrapper)
        self.app.put(
            "/api/spaces/{space_id}",
            ...
            description="Update an existing space with new configuration (requires complete object)",
            response_model=Space
        ...
        self.app.delete(
            ...
            description="Permanently delete a space and all its associated data",
        ...
        self.app.get(
            ...
            description="Retrieve a list of all users accessible to the current user",
        ...
        self.app.put(
            ...
            tags=["Users"],
            summary="Update User",
            description="Update an existing user account (requires complete object)",
            response_model=User
        )(update_user_wrapper)
        self.app.delete(
            "/api/users/{user_id}",
