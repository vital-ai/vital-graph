...
async def prop_sort_blocked(conn, space_id: str,
    ...
    try:
        ...
        logger.debug("prop_sort gate unreadable for %s: %s", space_id, exc)
        return True
...
async def fast_entity_prop_page(
    ...
    if terms is None:
        logger.debug("prop_sort: declining, unexpressible filter in %s", filters)
        return None
    if not terms and not sort_by:
        return None                      # the plain listing; not ours to serve

    built = build_page_sql(space_id, terms, sort_by,
                           descending=(sort_order or "asc").lower() == "desc",
                           typed=entity_type_uri is not None)
    if built is None:
        logger.debug("prop_sort: declining, unexpressible sort/filter shape")
        return None
    sql, params = built
    ...
    try:
        async with impl.db_impl.connection_pool.acquire() as conn:
            if await prop_sort_blocked(conn, space_id, entity_type_uri):
                logger.debug("prop_sort: declining, space/type blocked (%s)", space_id)
                return None
