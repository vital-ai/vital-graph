...
Provides commands for stats, search, fuzzy index management,
...
Usage:
    ...
    python entity_registry/entity_admin.py fuzzy status
    ...
    async def run(self, args):
        ...
        try:
            ...
            if cmd == 'stats':
                ...
                if sub == 'status':
                    await self.cmd_fuzzy_status(args)
                elif sub == 'sync':
                    await self.cmd_fuzzy_sync(args)
                elif sub == 'check':
                    await self.cmd_fuzzy_check(args)
                else:
                    print("Usage: entity_admin.py fuzzy {status|sync|check}")
            elif cmd == 'weaviate':
                if sub == 'status':
                ...
                print("Usage: entity_admin.py {stats|fuzzy|weaviate|search|export|types|delete|migrate}")
        finally:
...
def main():
    ...
    # fuzzy
    fuzzy_parser = subparsers.add_parser('fuzzy', help='MinHash LSH fuzzy index')
