#!/usr/bin/env python3
"""
Fuzzy Index Sync Script for Entity Registry.

Syncs entity data from PostgreSQL to the persistent fuzzy index
(Redis / AWS MemoryDB). For the in-memory backend this is a no-op
since that index rebuilds automatically on service startup.

Usage:
    python entity_registry/fuzzy_sync.py --full
    python entity_registry/fuzzy_sync.py --entity-id e_abc123
    python entity_registry/fuzzy_sync.py --full --dry-run
    python entity_registry/fuzzy_sync.py --status
    python entity_registry/fuzzy_sync.py --check
"""
...
async def show_status(fuzzy_index: EntityFuzzyIndex, pool: asyncpg.Pool):
    """Show current fuzzy index status."""
    async with pool.acquire() as conn:
        pg_count = await conn.fetchval(
            "SELECT COUNT(*) FROM entity WHERE status = 'active'"
        )

    backend = 'redis' if fuzzy_index.storage_config else 'memory'
    print("=" * 60)
    print("Fuzzy Index Status")
    print("=" * 60)
    print(f"  Backend:           {backend}")
    print(f"  Entities indexed:  {fuzzy_index.entity_count:,}")
    print(f"  PostgreSQL active: {pg_count:,}")
    print(f"  Initialized:       {fuzzy_index._initialized}")
    print(f"  Num permutations:  {fuzzy_index.num_perm}")
    print(f"  LSH threshold:     {fuzzy_index.threshold}")
    print(f"  Shingle k:         {fuzzy_index.shingle_k}")

    if backend == 'memory':
        print()
        print("  ℹ️  In-memory backend — index rebuilds on every service start.")
        print("     Use ENTITY_FUZZY_BACKEND=redis for persistent storage.")


async def check_consistency(fuzzy_index: EntityFuzzyIndex, pool: asyncpg.Pool):
    ...
    fuzzy_count = fuzzy_index.entity_count
    ...
    if pg_count == fuzzy_count:
        ...
        diff = abs(pg_count - fuzzy_count)
        print(f"  ⚠️  Mismatch: {diff:,} difference")
        print("  Run with --full to re-sync")


async def full_sync(fuzzy_index: EntityFuzzyIndex, pool: asyncpg.Pool,
                     dry_run: bool = False, since=None):
    """Full or incremental sync: PostgreSQL → fuzzy index."""
    mode = 'incremental' if since else 'full'
    logger.info("=" * 60)
    logger.info(f"Fuzzy Index {mode.title()} Sync")
    logger.info("=" * 60)
    ...
    logger.info(f"Current index count: {fuzzy_index.entity_count:,}")

    if dry_run:
        logger.info(f"Would sync {pg_count:,} entities to fuzzy index")
        return

    count = await fuzzy_index.initialize(pool, since=since)
...
async def rebuild_sync(fuzzy_index: EntityFuzzyIndex, pool: asyncpg.Pool,
                        dry_run: bool = False):
    ...
    """
    logger.info("=" * 60)
    logger.info("Fuzzy Index REBUILD (wipe + fresh load)")
    logger.info("=" * 60)
...
async def single_entity_sync(fuzzy_index: EntityFuzzyIndex, pool: asyncpg.Pool,
                              entity_id: str, dry_run: bool = False):
    """Sync a single entity to the fuzzy index."""
    ...
    if entity.get('status') == 'deleted':
        fuzzy_index.remove_entity(entity_id)
        logger.info(f"Removed {entity_id} from fuzzy index (entity is deleted)")
    else:
        fuzzy_index.add_entity(entity_id, entity)
        logger.info(f"Upserted {entity_id} to fuzzy index")


async def main():
    parser = argparse.ArgumentParser(
        prog='fuzzy_sync',
        description='Sync Entity Registry fuzzy index (Redis / MemoryDB)',
    )
    ...
    if backend == 'memory' and (args.full or args.rebuild or args.entity_id):
        logger.warning(
            ...
            "after this script exits. Set ENTITY_FUZZY_BACKEND=redis "
    ...
    try:
        ...
        if args.status or args.check:
            await fuzzy_index.initialize(pool, skip_lock=True)

            if args.status:
                await show_status(fuzzy_index, pool)
            if args.check:
                await check_consistency(fuzzy_index, pool)
        elif args.rebuild:
            await rebuild_sync(fuzzy_index, pool, dry_run=args.dry_run)
        elif args.full or args.since:
            await full_sync(fuzzy_index, pool, dry_run=args.dry_run,
                            since=since_dt)
        elif args.entity_id:
            # Initialize first so we can update incrementally
            await fuzzy_index.initialize(pool, skip_lock=True)
            await single_entity_sync(fuzzy_index, pool, args.entity_id,
