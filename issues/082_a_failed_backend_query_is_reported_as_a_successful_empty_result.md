...
## IT RECURRED, in code written AFTER the fix — 2026-08-13

The query-pipeline warm-up added the same day reported

    Query pipeline warm-up: 85 space(s) in 6156ms ... 0 skipped

while **9 of those 85 had failed**, each logging "v2 SQL generation failed:
relation ... does not exist". The warm-up called `execute_sparql_query` and
treated a normal return as success — exactly the mistake this issue is about,
made again by someone who had just fixed it.

Why: the fix here was applied to `kgquery_endpoint.py`, and the lesson was
internalised as "that endpoint checks the flag" rather than
"`execute_sparql_query` reports failure by FLAG, so EVERY caller must check it".

Two things follow:

1. the warm-up now checks `result.get("success") is False` and counts those as
   skipped, so its summary is truthful (76 spaces, 0 skipped);
2. **the general form is still open.** A function that reports failure by
   returning a dict rather than raising means every new caller must remember,
   and a caller that forgets gets silence. The durable fix is to make the
   failure impossible to ignore — raise by default, with an explicit
   `allow_failure=True` for the few paths that want the dict — rather than to
   keep auditing call sites.
