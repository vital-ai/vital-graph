...
| 096 | worked case | Not traversal work itself, but carries a tail-only pin priced BOTH ways on real data: selective-end-first is **2.9x**, the same form is **87x WORSE** when the entity is pinned. Win, regression, separating statistic and formulation all measured. `traversal_decision` needs only to count a type-constrained end as pinned, and something to read its answer. **2026-09-11:** the separating statistic as written picks WRONG on its own query (the winning case has MORE slots than entities, 5,726 vs 2,863); the real discriminator for the 87x case is `entity_uris`, which is syntactic and needs no stats |
