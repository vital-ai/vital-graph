...
| 200 | OPEN | `test_every_predicate_gets_its_biggest_before_any_gets_its_second` fails intermittently under `-n 4` for a reason unrelated to what it tests: `assert 1 >= 2`, "need at least two predicates". `test_space` is module-scoped, so predicates ACCUMULATE across a module's tests and this one needs a SIBLING to have left a second predicate; xdist's `--dist load` can hand it to a worker that ran none of them. Deterministic without parallelism — passes with the file in order, FAILS run alone. Its docstring treats the shared space as a feature, which is right for sizing thresholds and wrong as a precondition. Fix: have it insert its own second predicate. Found while verifying an unrelated `edge_fanout` change, which it is NOT caused by (reproduces with that reverted). |
