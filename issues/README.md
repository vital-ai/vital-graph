...
| 196 | OPEN | **One over-long identifier blocks EVERY index for the space.** `assert_identifiers_fit` refuses all index SQL when any generated name exceeds 63 bytes — right, since silent truncation collides — but it is all-or-nothing: one 65-byte name on `document_segmentation_config` denied `space_lead_dataset_test` its `frame_slot` indexes, which are 34-42 bytes and fit. Left half-migrated: table, 0 rows, 1 index against 7. Safe (the rewrite needs rows) but that gated fixture permanently loses the collapse — worth 4x10^11 on a multi-hop walk per `195`. Limit is 21 bytes and is undocumented where spaces are created |
