...
| 190 | OPEN | Both baselines promoted 2026-08-22 from a DIRTY tree; 126 commits to `vitalgraph/` since. Blocked on 188 and 189 by design  **2026-09-13: the suite now COMPLETES** — 3,564 s, 158 benches, and the run is a valid promotion candidate (class derived, promotion_blocked no, dirty False). Both preconditions met. **"Triage first" was WRONG:** `compare_bench` records a non-ok baseline entry as a known hole and FAILs only when an ok bench regresses, so promoting captures current state and improves from there — as intended. Real cost is 10 benches ok today that would not be, not 21 failures. Open question is the TIER SPLIT: the two baselines overlap in prefix so the rule cannot be recovered from the files | **RESOLVED for the query tier (`125d0c7b`):** full run promoted, compares clean against itself. **The split is not real** — all 51 `ingest.json` ids are inside the new `query.json`, and `ingest.json`'s whole content is 3 ok benches that the promotion also has ok; retire it. The 48 both held as `unrecorded` are `paging_fence_coverage`, whose file never calls `perf_record`, so they can never be ok in any pass. **New defect:** `query.partition.graph_scoped_pruning` vanished from the baseline (fixture error, not test failure) — a bench that errors in a fixture is never stamped and silently leaves the baseline.
