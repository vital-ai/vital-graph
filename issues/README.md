...
| **187** | OPEN | **`entity_slot_sort` is unmaintained by six write paths** — three in `kg_backend_utils`, three incremental importers. Each syncs `edge` and `frame_slot` in the same function and skips this one, with no stated reason. A stale row here is a WRONG SORT ORDER (`issues/096`: the sort reads its value straight off the table), not a slow query. Found by widening the `issues/185` matrix — the question that issue listed as NOT ESTABLISHED. Named in `KNOWN_GAPS`; measure drift per space before wiring |
