...
3. **A magic predicate in OUR namespace — `vg:listMember`** (decided
   ...
   is genuinely as cheap as the existing `*` handling, because it gives the
   emitter a NODE to match instead of a multi-node idiom to recognise — see
   `rdf_collections.md` §9.3-9.6. It also gives a natural place to lift the cap.

   Vendor-specific because there is **no standard equivalent**: RDF
   standardises the first/rest structure but no "member of this list" property,
   and `rdfs:member` is for CONTAINERS (`rdf:_n`) and needs RDFS entailment,
   which we do not implement. Since nothing is standard the URI is free — and
   it should be ours rather than Jena's `list:member`, because Jena is an
   internal detail no user knows about, so its URI buys portability nobody can
   use while committing us to ARQ's contract (`rdf_collections.md` §9.5).
